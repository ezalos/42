...
/*   Updated: 2019/01/14 16:26:52 by ldevelle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		save_file(t_list *gnl, char *buf, int read)
{
	...
	if (gnl->content != NULL)
		...
		size_save = 0;
	old_line = ft_strsub((char*)(gnl->content), 0, size_save);
	//ft_strdel((char **)&gnl->content);
	if (NULL == (gnl->content = (char*)ft_memalloc(sizeof(char) *
	(size_save + read + 1))))
		return (-2);
	if (size_save != 0)
		gnl->content = ft_strncpy((char*)gnl->content, old_line, (size_t)size_save);
